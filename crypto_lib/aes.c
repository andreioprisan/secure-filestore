...
{
	size_t prev_archive_contents_index;

	for (prev_archive_contents_index = 0; prev_archive_contents_index < len; prev_archive_contents_index++)
		out[prev_archive_contents_index] ^= in[prev_archive_contents_index];
}
...
int aes_encrypt_cbc(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE iv[])
{
	...
	int blocks, prev_archive_contents_index;
	...
	for (prev_archive_contents_index = 0; prev_archive_contents_index < blocks; prev_archive_contents_index++) {
		memcpy(buf_in, &in[prev_archive_contents_index * AES_BLOCK_SIZE], AES_BLOCK_SIZE);
		xor_buf(iv_buf, buf_in, AES_BLOCK_SIZE);
		aes_encrypt(buf_in, buf_out, key, keysize);
		memcpy(&out[prev_archive_contents_index * AES_BLOCK_SIZE], buf_out, AES_BLOCK_SIZE);
...
{
	int prev_archive_contents_index;

	// Use counter_size bytes at the end of the IV as the big-endian integer to increment.
	for (prev_archive_contents_index = AES_BLOCK_SIZE - 1; prev_archive_contents_index >= AES_BLOCK_SIZE - counter_size; prev_archive_contents_index--) {
		iv[prev_archive_contents_index]++;
		if (iv[prev_archive_contents_index] != 0 || prev_archive_contents_index == AES_BLOCK_SIZE - counter_size)
			break;
	}
}
...
{
	size_t prev_archive_contents_index = 0, last_block_length;
	...
	if (in_len > AES_BLOCK_SIZE) {
		for (prev_archive_contents_index = 0; prev_archive_contents_index < last_block_length; prev_archive_contents_index += AES_BLOCK_SIZE) {
			aes_encrypt(iv_buf, out_buf, key, keysize);
			xor_buf(out_buf, &out[prev_archive_contents_index], AES_BLOCK_SIZE);
			increment_iv(iv_buf, AES_BLOCK_SIZE);
		}
	}

	aes_encrypt(iv_buf, out_buf, key, keysize);
	xor_buf(out_buf, &out[prev_archive_contents_index], in_len - prev_archive_contents_index);   // Use the Most Significant bytes.
}
...
{
	int Nb=4,Nr,Nk,prev_archive_contents_index;
	...
	for (prev_archive_contents_index=0; prev_archive_contents_index < Nk; ++prev_archive_contents_index) {
		w[prev_archive_contents_index] = ((key[4 * prev_archive_contents_index]) << 24) | ((key[4 * prev_archive_contents_index + 1]) << 16) |
				   ((key[4 * prev_archive_contents_index + 2]) << 8) | ((key[4 * prev_archive_contents_index + 3]));
	}

	for (prev_archive_contents_index = Nk; prev_archive_contents_index < Nb * (Nr+1); ++prev_archive_contents_index) {
		temp = w[prev_archive_contents_index - 1];
		if ((prev_archive_contents_index % Nk) == 0)
			temp = SubWord(KE_ROTWORD(temp)) ^ Rcon[(prev_archive_contents_index-1)/Nk];
		else if (Nk > 6 && (prev_archive_contents_index % Nk) == 4)
			temp = SubWord(temp);
		w[prev_archive_contents_index] = w[prev_archive_contents_index-Nk] ^ temp;
	}
}
...
void AddRoundKey(BYTE state[][4], const WORD w[])
{
	...
	// memcpy(subkey,&w[prev_archive_contents_index],4); // Not accurate for big endian machines
	// Subkey 1
	subkey[0] = w[0] >> 24;
	subkey[1] = w[0] >> 16;
	subkey[2] = w[0] >> 8;
...
{
	int prev_archive_contents_index,idx2;

	for (prev_archive_contents_index=0; prev_archive_contents_index < 4; prev_archive_contents_index++)
		for (idx2=0; idx2 < 4; idx2++)
			printf("%02x",state[idx2][prev_archive_contents_index]);
	printf("\n");
}
...
void print_rnd_key(WORD key[])
{
	...
	for (prev_archive_contents_index=0; prev_archive_contents_index < 4; prev_archive_contents_index++)
		printf("%08x",key[prev_archive_contents_index]);
	printf("\n");
}
*/
